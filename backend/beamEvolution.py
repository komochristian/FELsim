...
class BeamEvolution:
    ...
    def add_sample(self, s: float, particles: Optional[np.ndarray] = None,
        ...
        ------
        ValueError
            If s-position already exists within tolerance
        """
    ...
    def update_sample(self, s: float, particles: Optional[np.ndarray] = None,
        ...
        ------
        KeyError
            If no s-position exists within tolerance
        """
    ...
    def _find_nearest_s(self, s: float) -> tuple[Optional[float], Optional[int]]:
        ...
        Returns
        -------
        nearest_s : float or None
            Nearest s-position in stored data, or None if empty
        index : int or None
            Index of nearest s-position, or None if empty
        """
...
# Example usage demonstrating the sorted behavior
if __name__ == "__main__":
    # Create evolution object
    evo = BeamEvolution(simulator_name="FELsim", beam_energy=45.0)

    # Add samples in non-sequential order - automatically maintained sorted
    evo.add_sample(1.5, particles=np.random.randn(100, 6))
    evo.add_sample(0.0, particles=np.random.randn(100, 6))
    evo.add_sample(3.0, particles=np.random.randn(100, 6))
    evo.add_sample(0.75, particles=np.random.randn(100, 6))

    print("s_positions (automatically sorted):", evo.s_positions)
    # Output: [0.0, 0.75, 1.5, 3.0]

    # Efficient nearest-neighbor lookup
    particles = evo.get_particles_at(1.49)  # Finds 1.5 within default tolerance
    print(f"Particles at s=1.49 (found nearest): {particles.shape if particles is not None else None}")

    # Try to add duplicate - raises error
    try:
        evo.add_sample(1.5, particles=np.random.randn(100, 6))
    ...
    evo.update_sample(1.5, particles=np.random.randn(100, 6))
    print("Updated sample at s=1.5")
