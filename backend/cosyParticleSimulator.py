"""
COSY INFINITY particle tracking simulator.
...
class COSYParticleSimulator(COSYSimulator):
    ...
    # --- Statistical analysis and Twiss ---
    ...
    def diagnose_particle_distribution(self, particles, coordinate_system='felsim'):
        ...
        self.logger.info(f"Particle Distribution Diagnostics ({coordinate_system.upper()} coordinates)")
        self.logger.info(f"Total particles: {N}")
        self.logger.info(f"Valid particles: {n_valid} ({100 * n_valid / N:.1f}%)")
        self.logger.info(f"Particles with NaN: {n_nan} ({100 * n_nan / N:.1f}%)")
        self.logger.info(f"Particles with Inf: {n_inf} ({100 * n_inf / N:.1f}%)")
        ...
        if n_valid > 0:
            ...
            self.logger.debug(f"Valid particle statistics:")
            self.logger.debug(f"{'Coordinate':<15} {'Min':>12} {'Max':>12} {'Mean':>12} {'RMS':>12}")
            self.logger.debug(f"{'-' * 70}")

            for i, name in enumerate(names):
                vals = valid[:, i]
                self.logger.debug(f"{name:<15} {np.min(vals):>12.6g} {np.max(vals):>12.6g} "
            ...
            if coordinate_system == 'cosy':
                ...
                self.logger.debug(f"COSY-specific checks:")
                self.logger.debug(f"  Particles with a² + b² > 0.1: {n_large} ({100 * n_large / n_valid:.1f}%)")
                self.logger.debug(f"  Particles with a² + b² > 1.0: {n_very_large} ({100 * n_very_large / n_valid:.1f}%)")
                self.logger.debug(f"  Particles with δK < -1 (negative KE): {n_neg_KE} "
                                  f"({100 * n_neg_KE / n_valid:.1f}%)")

                if n_very_large > 0:
                    self.logger.warning(f"{n_very_large} particles have unphysical momentum ratios — "
                                        f"may cause NaN in coordinate transformation")
        else:
            self.logger.warning("No valid particles found!")
