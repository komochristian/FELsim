...
class BeamlineBuilder:
        ...
        if not os.path.exists(excel_path):
            ...
            if found_path is None:
                raise FileNotFoundError(
                    f"Excel beamline file not found: {excel_path}\n"
                    f"Tried: {excel_path}, {os.path.join(os.getcwd(), excel_path)}"
                )
    ...
    def parse_beamline(self):
        ...
        for _, row in df.iterrows():
            ...
            if pd.notna(z_start) and pd.notna(z_end) and z_start < z_end:
                ...
                # print(f"Warning: Skipping invalid element {row['Element']} at row {_}")
                pass
        ...
        for key, value in kwargs.items():
            if isinstance(value, str):
                ...
                if len(value.strip()) > 50:
                    raise ValueError(f"String parameter '{key}' is too long (max 50 characters)")
    ...
    def set_current(self, index, current):
        ...
        if isinstance(current, str):
            ...
            if len(current.strip()) > 50:
                raise ValueError("String current is too long (max 50 characters)")
    ...
    def apply_variable_mapping(self, xVar, validation=True):
        ...
        Returns:
            dict - Summary with successful/failed operations and statistics
        """
        ...
        for index, modifications in xVar.items():
            try:
                ...
                if not isinstance(modifications, dict):
                    raise TypeError(f"Modifications for index {index} must be a dictionary, got {type(modifications)}")

                if not modifications:
                    raise ValueError(f"No modifications specified for index {index}")
                ...
                modified_element = self.modify_element(index, **modifications)
                ...
                self.logger.warning(f"Failed to apply modifications to element {index}: {e}")

        total_attempted = len(xVar)
        ...
        results["summary"] = {
            ...
            "failed": failed_count,
            "success_rate": successful_count / total_attempted if total_attempted > 0 else 0
        }

        if results["successful"]:
            self.logger.debug(
                f"Successfully applied variable mappings to {successful_count}/{total_attempted} elements")

        if results["failed"]:
            self.logger.debug(f"Failed to apply mappings to {failed_count} elements. Check warnings above for details.")
